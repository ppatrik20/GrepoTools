    ...
    var GameDataUnits = {
        ...
        getMaxBuild: function (units) {
            var town_id = Game.townId,
                ...
                unit_id, res = itown.resources(), max_build, gd_unit, gd_units = GameData.units,
                god_id = itown.god(), curr_favor = res.favor, unit_max_build,
                max = {}, factor;
        ...
        getDependencies: function (unit_id) {
            var itown = ITowns.getTown(Game.townId), town_buildings = itown.getBuildings(), town_researches = itown.getResearches(),
                gd_unit = GameData.units[unit_id], gd_researches = GameData.researches, gd_buildings = GameData.buildings,
                has_dependencies = false,
                building_dep = gd_unit.building_dependencies, research_dep = gd_unit.research_dependencies,
                building_id, i, l = research_dep.length;

            var left_building_dep = [], left_research_dep = [], research_id;
            ...
            return has_dependencies ? {buildings: left_building_dep, researches: left_research_dep} : {};
        },
        ...
        hasNavalUnits: function (unit_ids) {
            var i, naval_units = this.navalUnitIds(), naval_units_length = naval_units.length;
        ...
        hasGroundUnits: function (unit_ids) {
            var i, ground_units = this.groundUnitIds(), ground_units_length = ground_units.length;
         ...
         */
        allUnitIds: function () {
            var unit_id, all_unit_ids = [], units = GameData.units;
        ...
        navalUnitIds: function () {
            var unit_id, naval_unit_ids = [], units = GameData.units;
        ...
        groundUnitIds: function () {
            var unit_id, ground_unit_ids = [], units = GameData.units;
        ...
        flyingUnitIds: function () {
            var unit_id, flying_unit_ids = [], units = GameData.units;
		 ...
		 * @param has_berth				has berth/bunk research for more capacity
		 *
		 ...
		 *                                    will be needed to carry troops
		 */
        calculateCapacity: function (town_id, units, has_berth) {
            var gd_units = GameData.units, gd_unit,
                town = ITowns.getTown(town_id),
                berth = 0;

            if (town && town.getResearches().hasBerth() || has_berth) {
                berth = GameDataResearches.getBonusBerth();
            }
