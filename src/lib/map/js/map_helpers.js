...
define('map/helpers', function() {
	...
	var tileSizeBitWise = {x: 7, y: 7}, // 2 >> x
	...
	return {
		...
		/**
		 * Determines whether a town is a ghost town: a regular, abandoned town spot without an
		 * owning player. Island quest and other special spots also carry player_id null /
		 * player_name '', but they live on number_on_island >= USABLE_GAME_TOWN_SPOT_COUNT and are
		 * therefore SPECIAL_TOWN, never TOWN. The town type check keeps the custom ghost color from
		 * bleeding onto those spots.
		 *
		 * @param {Object} town Town object
		 * @return {Boolean}
		 */
		isGhostTown: function (town) {
			return this.getTownType(town) === TOWN_TYPES.TOWN && !town.player_id && town.player_name === '';
		},
